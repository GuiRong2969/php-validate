...
[![Php Version](https://img.shields.io/badge/php-%3E%3D8.1-brightgreen)](https://www.php.net/ChangeLog-8.php)
[![Latest Stable Version](https://img.shields.io/packagist/v/guirong/php-validate)](https://packagist.org/packages/guirong/php-validate)
[![Coverage Status](https://coveralls.io/repos/github/GuiRong2969/php-validate/badge.svg?branch=master)](https://coveralls.io/github/GuiRong2969/php-validate?branch=master)
![GitHub all releases](https://img.shields.io/github/downloads/GuiRong2969/php-validate/total)
...
- 独立的过滤器 `Guirong\Validate\Filter\Filtration`，可单独用于数据过滤
...
> 更多配置示例请参看 [像Laravel一样配置](#like-laravel)

## 项目地址

- **github** <https://github.com/GuiRong2969/php-validate.git>
...
- `2.x` 分支是支持 `php7.1+`的代码分支, [php7版本](#https://github.com/GuiRong2969/php-validate/tree/2.x)
- `1.x` 分支是支持 `php5` 的代码分支,但是基本上不再维护, [php5版本](#https://github.com/GuiRong2969/php-validate/tree/1.x)
...
需要快速简便的使用验证时，可直接使用 `Guirong\Validate\Validation`
...
创建一个新的class，并继承 `Guirong\Validate\Validation`。用于一个（或一系列相关）请求的验证, 相当于 laravel 的 表单请求验证
...
创建一个新的class，并使用 Trait `Guirong\Validate\ValidationTrait`。
...
<a name="like-laravel"></a>

## 像Laravel一样配置
`guirong/validate` 也支持像Laravel一样配置验证规则

- 每条规则验证一个字段，但是可以同时配置多个验证器
>如果你喜欢类似Yii方式的配置规则，也是可以的。 请参考前一篇文档：

### 方式1: 直接使用类 FieldValidation

需要快速简便的使用验证时，可直接使用 `guirong\Validate\FieldValidation`
```php
...
### 方式2: 继承类 FieldValidation

创建一个新的class，并继承 `Guirong\Validate\FieldValidation`。用于一个（或一系列相关）请求的验证, 相当于 laravel 的 表单请求验证
...
- 一个函数名称
- 一个当前验证类的方法名
  - 在继承了 `Guirong\Validate\Validation` 的子类添加验证方法. 请看上面的 [使用方式1](#how-to-use2)
...
> 别忘了继承 `\Guirong\Validate\Validator\AbstractValidator`,和实现必须方法`validate`
